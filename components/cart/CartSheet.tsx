...
import { Minus, Plus, ShoppingBag, Trash2, X, CreditCard, Loader2 } from "lucide-react";
...
export default function CartSheet() {
  ...
  const calculateShipping = async () => {
    ...
    try {
      const res = await fetch("/api/shipping", {
        ...
        body: JSON.stringify({ cepDestino: zipInput })
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || "Erro ao calcular");

      setShippingOptions(data.fretes);
      // Se tiver só 1 option ou queremos autoselecionar a primeira:
      // if(data.fretes.length > 0 && !shippingMethod) {
      //    setShipping(zipInput, Number(data.fretes[0].valor), data.fretes[0].nome);
      // }
    } catch (err: unknown) {
  ...
  const applyCoupon = () => {
    ...
    if (couponInput.trim().toUpperCase() === "HOOKE10") {
      setAppliedCoupon("HOOKE10");
      setCouponError("");
    } else {
  ...
  const discountValue = appliedCoupon === "HOOKE10" ? subtotal * 0.1 : 0;
      ...
      {isCheckoutOpen && items.length > 0 && (
        <div className="absolute inset-0 z-10 bg-white/95 backdrop-blur-sm flex flex-col p-6 overflow-y-auto w-full h-full">
          ...
          <div className="mt-8 flex-1 flex flex-col justify-center max-w-sm mx-auto w-full">
            ...
            <p className="text-sm text-gray-500 mb-8">Precisamos de dois dados rápidos para gerar seu pedido de <strong>{formatter.format(subtotal)}</strong> com segurança.</p>
              ...
              {!appliedCoupon ? (
                ...
                <div className="flex items-center justify-between border border-green-200 bg-green-50 px-3 py-2">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-green-800 uppercase">{appliedCoupon}</span>
                    <span className="text-[10px] text-green-700 bg-green-200 px-1 py-0.5 font-bold uppercase">10% OFF</span>
                  </div>
